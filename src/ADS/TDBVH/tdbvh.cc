...
TDBvh::TDBvh(const nlohmann::json& config) : Ads(config) { ConfigSetup(config); }
...
std::vector<std::shared_ptr<const Triangle>> TDBvh::Search(const Ray& r, bool first_hit) const {
    ...
    while (!s.empty()) {
        ...
        if (curr_node->isLeaf) {
            inci_ops += static_cast<uint32_t>(curr_node->triangle_indices.size());
...
void TDBvh::PrintStats(std::ostream& os) const {
    os << "Top Down BVH:\n";
    stats_.Print(os);
}
...
void TDBvh::CalculateStats() const {
    ...
    while (!s.empty()) {
        ...
        if (curr_node->isLeaf) {
            ...
            stats_.min_prims_per_leaf =
                std::min(stats_.min_prims_per_leaf, static_cast<uint32_t>(curr_node->triangle_indices.size()));
            stats_.max_prims_per_leaf =
                std::max(stats_.max_prims_per_leaf, static_cast<uint32_t>(curr_node->triangle_indices.size()));
            stats_.total_prims_per_leaf += curr_node->triangle_indices.size();
