...
BvhSeq2::BvhSeq2(const nlohmann::json& config) : Ads(config) {
    config_setup(config);
    reset_stats();
}
...
std::vector<std::shared_ptr<const Triangle>> BvhSeq2::Search(const Ray& r, bool first_hit) const {
    search_count_++;
...
void BvhSeq2::PrintStats(std::ostream& os) const {
...
void BvhSeq2::subdivide(size_t node_idx, int depth, AABB cb, const std::vector<Point3>& tcs,
...
BvhSeq2::BvhSeq2Stats BvhSeq2::calculate_stats() const {
    BvhSeq2Stats stats;
    stats.max_depth = 0;
...
void BvhSeq2::config_setup(const nlohmann::json& config) {
    std::clog << "Configuring sequential2 BVH..." << std::flush;

    max_triangles_per_BB_ = config.at("max_triangles_per_BB");
    max_depth_ = config.at("max_depth");
    bin_count_ = config.at("bin_count");

    std::clog << "\rSequential2 BVH configured     " << std::endl;
}
