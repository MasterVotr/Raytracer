...
BvhPar2V::BvhPar2V(const nlohmann::json& config) : Ads(config) {
    config_setup(config);
    reset_stats();
}

void BvhPar2V::Build(const std::vector<std::shared_ptr<const Triangle>>& triangles) {
    Ads::Build(triangles);
    reset_stats();
...
std::vector<std::shared_ptr<const Triangle>> BvhPar2V::Search(const Ray& r, bool first_hit) const {
    search_count_++;
...
void BvhPar2V::PrintStats(std::ostream& os) const {
...
float BvhPar2V::find_best_split(BvhNode& node, int& axis, float& split_pos, const AABB& cb, AABB& TB_L, AABB& TB_R,
...
void BvhPar2V::subdivide(size_t node_idx, int depth, AABB cb, const std::vector<Point3>& tcs,
...
void BvhPar2V::reset_stats() const {
    search_count_ = 0;
...
void BvhPar2V::config_setup(const nlohmann::json& config) {
    std::clog << "Configuring parallel2v BVH..." << std::flush;
