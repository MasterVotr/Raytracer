...
BvhVec::BvhVec(const nlohmann::json& config) : Ads(config) {
    config_setup(config);
    reset_stats();
}

void BvhVec::Build(const std::vector<std::shared_ptr<const Triangle>>& triangles) {
    Ads::Build(triangles);
    reset_stats();
...
#pragma omp simd reduction(min : CB_L_min_x, CB_L_min_y, CB_L_min_z) reduction(max : CB_L_max_x, CB_L_max_y, CB_L_max_z)
    ...
    std::cout << "\rVectorized BVH building time: " << duration / 1000.0 << " ms" << std::endl;
}

std::vector<std::shared_ptr<const Triangle>> BvhVec::Search(const Ray& r, bool first_hit) const {
    search_count_++;
...
void BvhVec::PrintStats(std::ostream& os) const {
...
BvhVec::BvhVecStats BvhVec::calculate_stats() const {
    BvhVecStats stats;
    stats.max_depth = 0;
...
void BvhVec::config_setup(const nlohmann::json& config) {
