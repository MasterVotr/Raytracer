...
Scene LoadScene(const nlohmann::json& loader_config, const nlohmann::json& scene_config) {
    ...
    bool ret =
        tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, filename.c_str(), basepath.c_str(), triangulate);
    ...
    for (size_t i = 0; i < shapes.size(); i++) {
        ...
        for (size_t f = 0; f < shapes[i].mesh.num_face_vertices.size(); f++) {
            ...
            // Lights
            if (materials[triangle.material_id].emission[0] != 0.0 ||
                materials[triangle.material_id].emission[1] != 0.0 ||
    ...
    for (size_t i = 0; i < materials.size(); i++) {
        ...
        material.transmittance =
            Vec3(materials[i].transmittance[0], materials[i].transmittance[1], materials[i].transmittance[2]);
